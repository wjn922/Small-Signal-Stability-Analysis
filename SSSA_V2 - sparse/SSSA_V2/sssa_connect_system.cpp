	...
	vector<int> gdim, vector<int> gl_gene, double **T, double **J)
{
	...
	for (int i = 1; i <= nd; i++)
	{
		for (int j = 1; j <= nd; j++)
		{
			J[dim_sys_all + i][dim_sys_all + j] = Ysystem[i][j];
		}
	}
	...
	for (int k = 0; k < num_gene; k++)
	{
		...
		J[row][dim_sys_all + col] = sin(Delta0);
		J[row][dim_sys_all + col + 1] = -cos(Delta0);
		J[row + 1][dim_sys_all + col] = cos(Delta0);
		J[row + 1][dim_sys_all + col + 1] = sin(Delta0);
		...
		J[dim_sys_all + row][col] = -1;
		J[dim_sys_all + row + 1][col + 1] = -1;
	}
